...
function GradCap({ style }: { style: React.CSSProperties }) {
  return (
    <svg width="44" height="34" viewBox="0 0 44 34" fill="none" xmlns="http://www.w3.org/2000/svg"
      style={{ ...style, filter: "drop-shadow(0 2px 8px rgba(255,215,0,0.4))" }}>
      <polygon points="22,3 44,14 22,25 0,14" fill="#FFD700" opacity="0.85"/>
      <ellipse cx="22" cy="25" rx="12" ry="6" fill="#006400" opacity="0.8"/>
      <rect x="38" y="14" width="3.5" height="14" rx="1.75" fill="#FFD700" opacity="0.9"/>
      <circle cx="39.75" cy="29" r="3.5" fill="#FFD700" opacity="0.9"/>
    </svg>
  )
}
...
function Star({ style }: { style: React.CSSProperties }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"
      style={{ ...style, filter: "drop-shadow(0 1px 4px rgba(255,215,0,0.6))" }}>
      <polygon points="9,1 11.5,6.5 18,7.5 13.5,11.5 14.5,18 9,15 3.5,18 4.5,11.5 0,7.5 6.5,6.5" fill="#FFD700" opacity="0.9"/>
    </svg>
  )
}
  ...
  return (
    <>
      ...
      <div style={{ width: "100vw", minHeight: "100vh", display: "flex", flexDirection: "column", fontFamily: "'Inter', 'Segoe UI', sans-serif", background: "#001a00", overflowX: "hidden" }}>
        ...
        {/* ══════════════════════════════════════════
            REAL SIET BANNER — full width, centered
            Replace /siet-logo.png with your real
            College Logo white Background-01 image
           ══════════════════════════════════════════ */}
        <div style={{ width: "100vw", background: "#fff", flexShrink: 0, display: "flex", justifyContent: "center", alignItems: "center", boxShadow: "0 4px 24px rgba(0,0,0,0.18)", position: "relative", zIndex: 20 }}>
          <img
            className="siet-banner-img"
            src="/siet-logo.png"
            alt="Sri Shakthi Institute of Engineering and Technology"
            style={{ width: "100%", maxWidth: "1400px", height: "auto", maxHeight: "110px", objectFit: "contain", objectPosition: "center", display: "block", padding: "4px 0" }}
          />
        </div>

        {/* Gold-green shimmer divider */}
        <div className="shimmer-bar" style={{ width: "100vw", height: "5px", flexShrink: 0 }} />
        ...
        {/* ══════════════════════════════════════════
            HERO — full screen, slideshow background
           ══════════════════════════════════════════ */}
        <div
          className="hero-section"
          style={{ flex: 1, width: "100vw", minHeight: "calc(100vh - 170px)", position: "relative", display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 16px", overflow: "hidden" }}
        >

          {/* Campus background with Ken Burns */}
          <div
            className="bg-layer"
            style={{ position: "absolute", inset: 0, backgroundImage: `url('${BG_IMAGES[bgIndex]}')`, backgroundSize: "cover", backgroundPosition: "center 30%", backgroundRepeat: "no-repeat", transition: "opacity 0.9s ease-in-out", opacity: bgVisible ? 1 : 0 }}
          />

          {/* Overlay — light so campus stays vivid */}
          ...
          <GradCap style={{ position: "absolute", top: "8%",  left: "5%",  animation: "float 4.5s ease-in-out infinite",         opacity: 0.22 }} />
          <GradCap style={{ position: "absolute", top: "12%", right: "6%", animation: "float 5.5s ease-in-out 1.2s infinite",     opacity: 0.2  }} />
          <GradCap style={{ position: "absolute", bottom: "22%", left: "8%",  animation: "float 4s ease-in-out 0.6s infinite",   opacity: 0.18 }} />
          <GradCap style={{ position: "absolute", bottom: "28%", right: "9%", animation: "float 5s ease-in-out 2s infinite",     opacity: 0.2  }} />
          <GradCap style={{ position: "absolute", top: "45%",  left: "3%",  animation: "float 3.8s ease-in-out 1.5s infinite",   opacity: 0.15 }} />
          <GradCap style={{ position: "absolute", top: "55%",  right: "4%", animation: "float 6s ease-in-out 0.8s infinite",     opacity: 0.15 }} />

          {/* Floating stars */}
          {[
            { top: "15%",  left: "18%",  delay: "0s",    dur: "3.2s" },
            { top: "30%",  right: "15%", delay: "1s",    dur: "4s"   },
            { top: "60%",  left: "14%",  delay: "0.4s",  dur: "3.8s" },
            { top: "70%",  right: "18%", delay: "1.8s",  dur: "4.5s" },
            { bottom: "35%", left: "30%", delay: "0.8s", dur: "3.5s" },
            { top: "20%",  left: "42%",  delay: "2.2s",  dur: "5s"   },
            { bottom: "18%", right: "30%", delay: "1.4s", dur: "4.2s" },
          ].map((s, i) => (
            <Star key={i} style={{ position: "absolute", ...s, animation: `starFloat ${s.dur} ease-in-out ${s.delay} infinite`, opacity: 0.28 }} />
          ))}
          ...
          <div
            className="siet-card"
            style={{ position: "relative", zIndex: 10, width: "100%", maxWidth: "430px", borderRadius: "20px", overflow: "hidden", backdropFilter: "blur(6px)" }}
          >
            {/* Shimmer top border */}
